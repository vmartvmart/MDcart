...
class Customer extends \MDcart\System\Engine\Controller {
	...
	private function getPayment(): string {
		...
		foreach ($results as $result) {
			...
			$data['payment_methods'][] = [
				'image'       => $image,
				'date_expire' => oc_jdate($this->language->get('date_format_short'), strtotime($result['date_expire'])),
	...
	public function getReward(): string {
		...
		foreach ($results as $result) {
			$data['rewards'][] = ['date_added' => oc_jdate($this->language->get('date_format_short'), strtotime($result['date_added']))] + $result;
		}
	...
	public function getAuthorize(): string {
		...
		foreach ($results as $result) {
			$data['authorizes'][] = [
				'date_added'  => oc_jdate($this->language->get('datetime_format'), strtotime($result['date_added'])),
				'date_expire' => $result['date_expire'] ? oc_jdate($this->language->get('date_format_short'), strtotime($result['date_expire'])) : '',
