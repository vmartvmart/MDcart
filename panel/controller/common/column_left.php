...
class ColumnLeft extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/product_import_url')) {
				$catalog[] = [
					'name'     => $this->language->get('text_import_product_url'),
			...
			if ($this->user->hasPermission('access', 'catalog/warehouse_transfer')) {
				$catalog[] = [
			...
			if ($this->user->hasPermission('access', 'design/theme_switcher')) {
				$design[] = [
					'name'     => $this->language->get('text_theme_switcher'),
			...
			if ($this->user->getGroupId() === 1 && $this->user->hasPermission('access', 'design/theme_manager')) {
				$design[] = [
					'name'     => $this->language->get('text_theme_manager'),
			...
			if ($this->user->hasPermission('access', 'sale/pos')) {
				$sale[] = [
			...
			if ($this->user->hasPermission('access', 'sale/preorder')) {
				$sale[] = [
			...
			// Accounting - a full double-entry bookkeeping module (Chart of
			// Accounts, Bank/Cash Accounts, Journal Entries with quick
			// Expense/Receipt/Payment forms, General Ledger, Trial Balance).
			// Every item here is gated by its own permission, same as the
			// rest of this menu - nothing shows unless an admin has granted
			// it (System > Users > User Groups).
			$accounting = [];
			...
			if ($this->user->hasPermission('access', 'accounting/bank_account')) {
				$accounting[] = [
			...
			if ($this->user->hasPermission('access', 'accounting/report')) {
				$accounting[] = [
					'name'     => $this->language->get('text_general_ledger'),
			...
			if ($this->user->hasPermission('access', 'accounting/exchange_rate')) {
				$accounting[] = [
			...
			// Hidden from the menu on purpose: this built-in item only checks the
			// official update server for core releases, unrelated to our own
			// repository-based update below — showing both was confusing. The
			// route itself is untouched, just not advertised in the menu.
			//
			...
			// GitHub-based distribution updater (task: private-repo update button)
			if ($this->user->hasPermission('access', 'tool/system_update')) {
				$maintenance[] = [
					'name'     => $this->language->get('text_system_update'),
			...
			// Profit/Loss - deliberately not granted to every user group by the
			// install scripts; only shows for whichever group(s) an admin has
			// explicitly given the report/profit_loss permission to.
			if ($this->user->hasPermission('access', 'report/profit_loss')) {
				$report[] = [
					'name'     => $this->language->get('text_profit_loss'),
