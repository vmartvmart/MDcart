...
class Product extends \MDcart\System\Engine\Controller {
	...
	public function form(): void {
		...
		// Warehouse stock/cost breakdown - oc_product.quantity above is kept in sync
		// with whichever warehouse is currently the selling warehouse.
		$this->load->model('catalog/warehouse');

		$data['product_warehouses'] = $this->model_catalog_warehouse->getProductWarehouses(!empty($product_info) ? (int)$product_info['product_id'] : 0);
		...
		// Pre-order / backorder
		if (!empty($product_info)) {
			$data['preorder_status'] = $product_info['preorder_status'];
		} else {
			$data['preorder_status'] = 0;
		}

		if (!empty($product_info) && $product_info['preorder_lead_days'] !== null) {
			$data['preorder_lead_days'] = $product_info['preorder_lead_days'];
		} else {
	...
	public function save(): void {
		...
		if (!$json) {
			...
			// Warehouse stock/cost breakdown
			$product_id = !empty($post_info['product_id']) ? (int)$post_info['product_id'] : (int)($json['product_id'] ?? 0);

			if ($product_id && !empty($post_info['warehouse_stock']) && is_array($post_info['warehouse_stock'])) {
				$this->load->model('catalog/warehouse');

				foreach ($post_info['warehouse_stock'] as $warehouse_id => $stock) {
					$this->model_catalog_warehouse->setProductWarehouseStock($product_id, (int)$warehouse_id, (int)($stock['quantity'] ?? 0), (float)($stock['cost_price'] ?? 0));
				}
			}
	...
	public function getReport(): string {
		...
		foreach ($results as $result) {
			...
			$data['reports'][] = [
				...
				'date_added' => oc_jdate($this->language->get('datetime_format'), strtotime($result['date_added']))
			];
		}
	...
	public function autocomplete(): void {
		...
		foreach ($results as $result) {
			...
			$json[] = [
				...
				'preorder_status'  => !empty($result['preorder_status'])
			];
		}
