 ...
 */
class Fraud extends \MDcart\System\Engine\Controller {
	/**
	 * Index
	 *
	 * @return void
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$data['extensions'][] = [
					...
					'status'    => $this->config->get('fraud_' . $code . '_status'),
	...
	public function uninstall(): void {
		...
		if (!$json) {
			...
			$this->model_setting_extension->uninstall('fraud', $code);

			// Call uninstall method if it exists
			$this->load->controller('extension/' . $extension . '/fraud/' . $code . '.uninstall');
