...
class Payping extends \Opencart\System\Engine\Model {
	...
	public function getMethods(array $address = []): array {
		...
		if (!$this->config->get('payment_payping_api_key')) {
			return [];
		}
		...
		if ($this->cart->hasSubscription()) {
			...
			$status = true;
		} elseif (!$this->config->get('payment_payping_geo_zone_id')) {
			$status = true;
		} else {
			$this->load->model('localisation/geo_zone');

			$results = $this->model_localisation_geo_zone->getGeoZone((int)$this->config->get('payment_payping_geo_zone_id'), (int)$address['country_id'], (int)$address['zone_id']);
