 ...
 * Class Order
 *
 * Sends Telegram notifications for new orders and order status changes.
 *
...
class Order extends \Opencart\System\Engine\Controller {
	...
	public function index(string &$route, array &$args): void {
		if (!$this->config->get('other_telegram_status') || !$this->config->get('other_telegram_bot_token')) {
			return;
		}
		...
		if (!$order_info['order_status_id'] && $order_status_id) {
			...
			if ($this->config->get('other_telegram_order_add_status')) {
				$chat_id = $this->getLinkedChatId($order_id);

				if ($chat_id) {
					$telegram->sendMessage($chat_id, sprintf($this->language->get('text_order_add'), $order_info['order_id'], $total));
				}
			}
		...
		// Status change
		if ($order_info['order_status_id'] && $order_status_id && $notify && $this->config->get('other_telegram_order_status_status')) {
			$chat_id = $this->getLinkedChatId($order_id);
			...
			if ($chat_id) {
				...
				$telegram->sendMessage($chat_id, sprintf($this->language->get('text_order_status'), $order_info['order_id'], $order_status_info['name'] ?? ''));
			}
		}
	}
