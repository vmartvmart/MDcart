 ...
 * Class Telegram
 *
 * Public endpoint Telegram POSTs Update objects to (registered via setWebhook).
 * Handles the "/start <token>" deep-link payload sent when a user/admin taps
...
class Telegram extends \Opencart\System\Engine\Controller {
	...
	private function linkAdmin(string $token, string $chat_id): void {
		if (!$token || $token !== (string)$this->config->get('other_telegram_admin_link_token')) {
			return;
		}

		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'other_telegram' AND `key` = 'other_telegram_admin_chat_id'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `store_id` = '0', `code` = 'other_telegram', `key` = 'other_telegram_admin_chat_id', `value` = '" . $this->db->escape($chat_id) . "', `serialized` = '0'");
	...
	private function linkOrder(string $token, string $chat_id): void {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "notify_link` WHERE `channel` = 'telegram' AND `purpose` = 'order' AND `token` = '" . $this->db->escape($token) . "'");
		...
		if ((int)$query->row['customer_id'] > 0) {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "notify_link` WHERE `channel` = 'telegram' AND `purpose` = 'customer' AND `order_id` = '" . (int)$query->row['customer_id'] . "'");
			$this->db->query("INSERT INTO `" . DB_PREFIX . "notify_link` SET `channel` = 'telegram', `token` = '" . $this->db->escape(bin2hex(random_bytes(16))) . "', `order_id` = '" . (int)$query->row['customer_id'] . "', `customer_id` = '" . (int)$query->row['customer_id'] . "', `purpose` = 'customer', `chat_id` = '" . $this->db->escape($chat_id) . "', `status` = '1', `date_added` = NOW()");
		}
	...
	private function linkStockAlert(string $token, string $chat_id): void {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "stock_alert` WHERE `channel` = 'telegram' AND `token` = '" . $this->db->escape($token) . "'");
