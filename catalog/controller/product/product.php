...
class Product extends \MDcart\System\Engine\Controller {
	...
	public function index(): ?\MDcart\System\Engine\Action {
		...
		if ($product_info) {
			...
			$data['preorder_status'] = !empty($product_info['preorder_status']);

			// Sellable while in transit (task #13): a concrete alternative to
			// the generic pre-order promise above, backed by a real incoming
			// warehouse transfer the admin has explicitly opted in
			// (presell_online) - takes priority over the generic pre-order
			// message when both would otherwise apply, since it comes with an
			// actual delivery estimate rather than a rough lead time.
			$data['transit_available'] = false;
			$data['transit_message'] = '';
			$data['transit_quantity'] = 0;

			if ($data['out_of_stock']) {
				$this->load->model('catalog/product');

				$transit_options = $this->model_catalog_product->getTransitAvailability($product_id);

				if ($transit_options) {
					$best = $transit_options[0];

					$data['transit_available'] = true;
					$data['transit_quantity'] = $best['available'];

					if ($best['delivery_date']) {
						$delivery_date = new \DateTime($best['delivery_date']);
						$today = new \DateTime('today');
						$days_left = max(0, (int)$today->diff($delivery_date)->format('%r%a'));

						$data['transit_message'] = sprintf($this->language->get('text_transit'), $delivery_date->format($this->language->get('date_format_short')), $days_left);
					} else {
						$data['transit_message'] = $this->language->get('text_transit_no_date');
					}

					// The transit offer always takes priority when both are
					// available for this product - don't show two competing
					// out-of-stock options at once.
					$data['preorder_status'] = false;
					$data['preorder_message'] = '';
				}
			}

			if ($data['out_of_stock'] && $data['preorder_status']) {
				$preorder_lead_days = !empty($product_info['preorder_lead_days']) ? (int)$product_info['preorder_lead_days'] : 7;
				$data['preorder_message'] = sprintf($this->language->get('text_preorder'), $preorder_lead_days);
			} else {
				$data['preorder_message'] = '';
			}
	 ...
	 */
	/**
	 * Renders the "Notify me when available" widget (extension/stock_alert)
	 * for an out-of-stock product, or an empty string if that extension isn't installed/enabled.
	 *
	...
	private function getStockAlertWidget(int $product_id): string {
		...
		if (!$data['status'] || (!$data['phone'] && !$data['telegram'] && !$data['bale'])) {
			return '';
		}
		...
		$data['product_id'] = $product_id;
		$data['subscribe_url'] = $this->url->link('extension/stock_alert/product/stock_alert.subscribe', 'language=' . $this->config->get('config_language'));
		...
		return $this->load->view('extension/stock_alert/product/stock_alert', $data);
	}
