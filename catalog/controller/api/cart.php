...
class Cart extends \Opencart\System\Engine\Controller {
	...
	public function addProduct(): array {
		...
		if ($product_info) {
			...
			$out_of_stock = (!$product_info['quantity'] || ($product_info['quantity'] < $product_total + $quantity));

			// Pre-order / backorder: admin approval is a per-product decision
			// (Catalog > Products > "Allow Pre-Order", preorder_status), never
			// just "this request came from POS" - the same flag gates both
			// POS and the storefront equally, so once a product is approved a
			// cashier can sell it as a pre-order (in stock or not, any
			// delivery date they choose) and a customer can self-service
			// pre-order it too (still only once it's actually out of stock).
			$want_preorder = !empty($this->request->post['preorder']);
			$product_preorder_allowed = !empty($product_info['preorder_status']);
			$can_preorder = $want_preorder && $product_preorder_allowed;

			if ($want_preorder && !$product_preorder_allowed) {
				// Cashier/customer asked for a pre-order on a product the
				// admin hasn't approved for it - reject with a specific
				// reason rather than silently selling it as a normal line
				// (in stock) or falling through to the generic stock error
				// (out of stock, which would read as "just out of stock",
				// not "pre-order isn't allowed here").
				$output['error']['warning'] = $this->language->get('error_preorder_not_allowed');
			} elseif (!$this->config->get('config_stock_checkout') && $out_of_stock && !$can_preorder) {
		...
		if (!$output) {
			...
			// Applies whenever the line is validly flagged as a pre-order, not
			// only when the product happens to be out of stock right now -
			// POS cashiers can deliberately sell an in-stock, admin-approved
			// product on a later delivery date too (the storefront's own
			// separate gate in checkout/cart.php still requires the product
			// to actually be out of stock for a customer's self-service
			// pre-order).
			if (!empty($can_preorder)) {
				$preorder_delivery_date = (string)($this->request->post['preorder_delivery_date'] ?? '');

				if (!$preorder_delivery_date && !empty($product_info['preorder_lead_days'])) {
					$preorder_delivery_date = date('Y-m-d', strtotime('+' . (int)$product_info['preorder_lead_days'] . ' days'));
				}

				// Force stock_status true so Cart::hasStock() (which otherwise
				// blocks storefront checkout confirmation) treats this line as
				// fine - it's deliberately being sold/ordered ahead of stock.
				$override['stock_status'] = true;
				$override['is_preorder'] = 1;
				$override['preorder_delivery_date'] = $preorder_delivery_date ?: null;
			}
