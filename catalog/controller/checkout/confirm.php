...
class Confirm extends \MDcart\System\Engine\Controller {
	...
	public function index(): string {
		...
		if ($status) {
			...
			$payment_currency = $this->session->data['currency'];

			// Customers can browse and compare prices in toman. Payment gateways,
			// however, receive Iranian amounts in rial (1 toman = 10 rials).
			// Store the order in IRR so a gateway that uses the order currency and
			// value receives the correct rial amount, while storefront rendering
			// continues to use the visitor's IRT session currency.
			if ($payment_currency === 'IRT') {
				$payment_currency = 'IRR';
			}

			$order_data['currency_id'] = $this->currency->getId($payment_currency);
			$order_data['currency_code'] = $payment_currency;
			$order_data['currency_value'] = $this->currency->getValue($payment_currency);
