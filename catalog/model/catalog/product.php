...
class Product extends \Opencart\System\Engine\Model {
	...
	/**
	 * Get Transit Availability (task #13 - "sellable while in transit")
	 *
	 * Every `in_transit` warehouse transfer heading to the store's selling
	 * warehouse that the admin has explicitly marked `presell_online`, still
	 * has spare (unreserved) quantity for this product, sorted soonest-
	 * arriving first. Used to let the storefront offer this product for sale
	 * even while it's genuinely out of stock in the selling warehouse, with a
	 * concrete promised delivery date - and, since each transfer is capped at
	 * its own actual incoming quantity, this can never oversell more than
	 * what is really on its way.
	 *
	 * A single order line is only ever allocated against ONE transfer (see
	 * catalog/controller/checkout/cart.php::add()), so this intentionally
	 * does not merge quantity across multiple transfers - a customer wanting
	 * more than the soonest transfer alone can cover simply doesn't get the
	 * transit option for that quantity, and needs to order less at a time.
	 *
	 * @param int $product_id
	 *
	 * @return array<int, array<string, mixed>> each row: transfer_id, available (int), delivery_date (Y-m-d)
	 */
	public function getTransitAvailability(int $product_id): array {
		$query = $this->db->query("SELECT `t`.`transfer_id`, `t`.`estimated_delivery_date`, (`tp`.`quantity` - `tp`.`reserved_quantity`) AS `available` FROM `" . DB_PREFIX . "warehouse_transfer` `t` INNER JOIN `" . DB_PREFIX . "warehouse_transfer_product` `tp` ON (`tp`.`transfer_id` = `t`.`transfer_id`) INNER JOIN `" . DB_PREFIX . "warehouse` `w` ON (`w`.`warehouse_id` = `t`.`to_warehouse_id`) WHERE `w`.`is_selling_warehouse` = '1' AND `t`.`status` = 'in_transit' AND `t`.`presell_online` = '1' AND `tp`.`product_id` = '" . (int)$product_id . "' AND (`tp`.`quantity` - `tp`.`reserved_quantity`) > 0 ORDER BY `t`.`estimated_delivery_date` ASC, `t`.`transfer_id` ASC");
		...
		foreach ($query->rows as $row) {
			$result[] = [
				...
				'delivery_date' => $row['estimated_delivery_date']
			];
		}
